...
// Version of chain with trimming, and rewritten to avoid recursion (just in
// case we need to detect more than ~10.000 SCCs).
pub fn chain_saturation_trim(
    graph: &SymbolicAsyncGraph,
...
// Version of chain with trimming, and rewritten to avoid recursion (just in
// case we need to detect more than ~10.000 SCCs).
pub fn chain_saturation_trim_hamming(
    graph: &SymbolicAsyncGraph,
) -> impl Iterator<Item = GraphColoredVertices> {
    precondition_graph_not_colored(graph);
